...
+ (void)signPassWithURL:(NSURL *)passURL certSuffix:(NSString*)certSuffix outputURL:(NSURL *)outputURL zip:(BOOL)zip {
    ...
    NSString *originFileList = @"";
    // For each file in the pass directory...
    for (NSURL *theURL in enumerator) {
        fileIndex++;
        originFileList = [originFileList stringByAppendingFormat:@"    \"%@\",\n", [[theURL.absoluteString pathComponents] lastObject]];
        ...
        if ([relativePath.uppercaseString hasSuffix:@".DS_STORE"]) {
            continue;
        }
    ...
    if (fileIndex > 0) {
        PSPrintLine(@"\n----------------------------------------------------------------------");
        PSPrintLine(@"Original Files(count:%d):\n{", fileIndex);
        PSPrintLine(@"%@}", originFileList);
        PSPrintLine(@"----------------------------------------------------------------------");
    }
    ...
    NSInteger allowedFileCount = manifestDictionary.allKeys.count;
    if (allowedFileCount > 0) {
        PSPrintLine(@"Allowed Files(count:%lu):\n{", allowedFileCount);
    }
    for (NSString *key in manifestDictionary.allKeys) {
        NSString *value = manifestDictionary[key];
        PSPrintLine(@"    \"%@\" = %@", key, value);
    }
    if (allowedFileCount > 0) {
        PSPrintLine(@"}");
        PSPrintLine(@"----------------------------------------------------------------------");
    ...
    if (status != noErr) {
        ...
        BOOL result = [signedData writeToURL:signature atomically:YES];
        if (!result) {
            NSLog(@"signature file write error!");
            return;
        }
    }
    ...
    PSPrintLine(@"Sign Result: Success!");
    PSPrintLine(@"----------------------------------------------------------------------");
}
